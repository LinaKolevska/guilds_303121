...
* 2.2.1) We now proceed with examining the Numerical Columns, by visualizing the distributions of the columns to uncover patterns, outliers, and data properties.
  ...
  ![PHOTO-2024-12-04-01-23-06](https://github.com/user-attachments/assets/2f6e9d6d-0ed2-444e-a395-7c5544f2f3de)
...
* ![PHOTO-2024-12-04-01-23-08](https://github.com/user-attachments/assets/63283885-e5c4-46b7-bafd-534df0b290ca)
...
* 2.5.3) Doing KNN first for numerical continuous and discrete and THEN encoding the categorical values utilizing custom mapping to do KNN for categorical columns.
	* **KNN imputation for numerical columns:**
     ...
     * **KNN imputation for categorical variables:** Firstly, we *inspect the categorical columns* by displaying unique values for each column to understand the range of possible values; done with `f"{col}: {df_balanced[col].unique()}".` This helps in preparing for the next steps, which are encoding and imputation. As aforementioned, encoding is a necessary step in imputing categorical variables as KNN works with numerical data.

Our encoding function first cleaned the values by converting all values to lowercase and removing the extra whitespaces. Then, it maps categorical values to binary `df_encoded[col].map({'yes': 1, 'present': 1, 'no': 0, 'absent': 0})`. The final step of the encoding function is handling missing values by replacing them with -1 as a placeholder.
...
The ancient secrets of magic, told by the wind, and the stars that trace the fate of wizards, were put before us in Marendor where a grand undertaking was entrusted to us: find out who would make it to the great Master Guild of Magic. With wit, creativity, and unflinching drive, we have built a piece of predictive science, a system that decodes guild membership with utmost accuracy.

Thus, our journey started with the raw and unpolished stories of thousands of scholars who came  to the academy with their dreams, and hopes, and magical abilities, which were represented as data.  We initiated a very careful process to create order out of the chaos and used the best tools of data  science like it was some sort of a sorcery. Using correlation analysis, chi-squared tests and  outlier detection we refined the data and turned it into a precious stone – a clean royal dataset. 

The core of our project was the Random Forest, the jewel of ML models. It won the war like a good and just judge when challenged against formidable adversaries: Gradient Boosted Trees, Logistic Regression, and K-Nearest Neighbors. The Random Forest was ready with training, validation and sleight-of-hand hyperparameter tuning, and it lit up the way to the Master Guild with dazzling sharpness and yet flexibility.

When we proceeded to explore the mystical  world of scholar attributes some of the traits turned out to be powerful: such as the mystical Fae Dust Reserve, the boundless Mystic Energy Level, and the enigmatic Mystical Index. They were carved well into the rocks of Mystical Marendor; these features were   the keys to guild membership, and helped us identify the real wizards. 

But ‘tis was not enough to just predict!! With the accuracy of a cartographer, we traced out the intricate correlations between a scholar’s character and their inexorable fate. Each insight became a lamp, revealing what makes the worthy shine from the others and providing a roadmap for the future of the kingdom.

‘Tis not merely a data set system—it is legacy. An adaptable structure, t’was constantly evolving to guarantee that the Master Guild is populated by the most deserving wizards, all the while cultivating a culture of excellence and aspiration in magical society. The kingdom of Marendor now finds itself on the brink of a golden age, where magic and data intertwine to safeguard its future.

My Lord, we did not only do your command but we did it as a piece of art of innovative and visionary. And as the sun sets on this chapter, the dear Marendor kingdom shall shines brighter than ever, its future etched in the stars and guided by the wisdom of data.
